...
alter table public.survey_questions
  drop constraint if exists survey_questions_section_id_fkey;

alter table public.survey_questions
  add constraint survey_questions_section_id_fkey
  foreign key (section_id) references public.survey_sections(id) on delete set null;
...
for all to authenticated using (
  app.is_admin() or created_by = auth.uid()
) with check (
  app.is_admin() or created_by = auth.uid()
);
...
drop trigger if exists set_survey_folders_updated_at on public.survey_folders;
create trigger set_survey_folders_updated_at before update on public.survey_folders
